...
// The client must PUT with the exact Content-Type header used to sign.
export async function POST(req: NextRequest) {
  ...
  const storageKey = `digital-products/${session.publisherId}/${Date.now()}-${safeFilename}`
  ...
  return NextResponse.json({ uploadUrl, storageKey, expiresIn: expires })
}
